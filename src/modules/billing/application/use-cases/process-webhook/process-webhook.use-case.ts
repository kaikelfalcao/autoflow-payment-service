...
export class ProcessWebhookUseCase {
  ...
  async execute(input: ProcessWebhookInput): Promise<void> {
    ...
    const charge = await this.charges.findByServiceOrderId(
    ...
    if (payment.status === "approved") {
      if (charge.status !== "PENDING") {
        this.logger.log(
          `Charge ${charge.id.value} already processed (${charge.status}), skipping approve`,
        );
        return;
      }
      ...
      if (charge.status !== "PENDING") {
        this.logger.log(
          `Charge ${charge.id.value} already processed (${charge.status}), skipping reject`,
        );
        return;
      }
      ...
      if (charge.status !== "APPROVED") {
        this.logger.log(
          `Charge ${charge.id.value} is not APPROVED (${charge.status}), skipping refund`,
        );
        return;
      }
      charge.refund();
