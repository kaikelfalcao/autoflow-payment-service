...
Given("o Billing Service está operacional", function (this: BillingWorld) {
  assert.ok(
    this.createChargeUseCase,
    "CreateChargeUseCase should be available",
  );
});
...
Given(
  "o valor total é {int} centavos",
  function (this: BillingWorld, cents: number) {
    this.currentTotalCents = cents;
  },
);

Given(
  "o cliente possui id {string}",
  function (this: BillingWorld, customerId: string) {
    this.currentCustomerId = customerId;
  },
);

When(
  "o Billing Service processa o evento de orçamento",
  async function (this: BillingWorld) {
    ...
    this.currentCharge = await this.repo.findByServiceOrderId(
...
Then(
  ...
  function (this: BillingWorld, eventType: string, orderId: string) {
    ...
    assert.ok(
      event,
      `Event "${eventType}" for order "${orderId}" should have been published`,
    );
  },
);
...
Then(
  ...
  function (this: BillingWorld, orderId: string) {
    ...
    assert.strictEqual(
      count,
      1,
      "There should be exactly one charge per order",
    );
  },
);
